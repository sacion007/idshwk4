...
event zeek_init()
{
	local reducer1 = SumStats::Reducer($stream = "all_response",$apply = set(SumStats::SUM));
	local reducer2 = SumStats::Reducer($stream = "404_response",$apply = set(SumStats::SUM));
	local reducer3 = SumStats::Reducer($stream = "404_url",$apply = set(SumStats::UNIQUE));
